...
full_mpaa %>% 
  ...
  labs(title = "Proportion of Movies with Short Reasons Generally on the Decline", 
       y = "",
       color = "Rating",
       x = "")
```
...
```{r}
pg_unigrams <- wc_unigrams %>% 
  filter(rating == "PG")

pg13_unigrams <- wc_unigrams %>% 
  filter(rating == "PG-13")
```
...
```{r}
noun <- "sexual"
pattern <- glue("(?<=, |for )(.+ and)?( for)?([^,]*?{noun}[^,]*?)(.|,.+?| and.+?)?$")

modifier_mpaa <- full_mpaa %>% 
  filter(rating == "PG",
    grepl(noun, reason)) %>% 
  mutate(reason = str_trim(reason),
         modifiers = str_match(reason, pattern)[ ,4]) %>% 
...
modifier_mpaa %>% 
  ...
  arrange(desc(n)) %>%
  ggplot(aes(x = mod_count, reorder(modifiers, n), fill= modifiers)) +
